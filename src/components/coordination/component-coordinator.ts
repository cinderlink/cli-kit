...
import { EventBus, BaseEvent } from '../../core/event-bus'
  ...
  startCoordination(
    ...
    return Effect.gen(function* () {
      ...
      for (const eventType of pattern.eventTypes) {
        const unsubscribe = yield* this.eventBus.subscribe<BaseEvent>(
          eventType,
...
export const MasterDetailPattern: CoordinationPattern = {
  ...
  initialize: (coordination) => Effect.sync(() => {
    console.log(`Master-Detail coordination initialized for ${coordination.participants.join(', ')}`)
  }),
  
  handleEvent: (event, coordination) => Effect.gen(function* () {
    const eventBus = (coordination as any).eventBus // TODO: Better pattern access
    ...
    if (event.type === 'component-selection') {
      ...
      if ((event as any).componentId === masterId) {
        ...
        yield* eventBus.publish('component-update', {
          ...
          payload: {
            targetId: detailId,
            selectedItem: (event as any).selectedItem
          }
        })
      }
    }
  ...
  isEventRelevant: (event, coordination) => {
    const componentId = (event as any).componentId
    return coordination.participants.includes(componentId)
  },
...
export const DataFlowPattern: CoordinationPattern = {
  ...
  initialize: (coordination) => Effect.sync(() => {
    console.log(`Data flow coordination initialized for ${coordination.participants.join(' -> ')}`)
  }),
  
  handleEvent: (event, coordination) => Effect.gen(function* () {
    if (event.type === 'data-changed') {
      const sourceIndex = coordination.participants.indexOf((event as any).componentId)
  ...
  isEventRelevant: (event, coordination) => {
    return coordination.participants.includes((event as any).componentId)
  },
