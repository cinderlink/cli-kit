...
export const modal = (config: Partial<ModalConfig> = {}): {
  ...
  view(model: ModalModel): View {
    ...
    const backdropStyle = model.config.backdropStyle ?? defaultConfig.backdropStyle!
    const modalWidth = model.config.width ?? defaultConfig.width!
    const modalHeight = model.config.height ?? defaultConfig.height!
