...
describe("Plugin Validation", () => {
  describe("validatePlugin", () => {
    ...
    test("validates plugin name format", () => {
      ...
      const result = validatePlugin(plugin)
      expect(result.valid).toBe(false)
      expect(result.errors).toContain("Plugin name must be lowercase alphanumeric with hyphens only")
    })
    ...
    test("validates command names", () => {
      ...
      const result = validatePlugin(plugin)
      expect(result.valid).toBe(false)
      expect(result.errors).toContain("Command name 'Invalid Command' must be lowercase alphanumeric with hyphens only")
    })
    ...
    test("validates extension paths", () => {
      ...
      const result = validatePlugin(plugin)
      expect(result.valid).toBe(false)
      expect(result.errors).toContain("Extension path 'invalid' must target a specific command (e.g., 'app.build')")
    })
  ...
  describe("checkPluginCompatibility", () => {
    test("checks version compatibility", () => {
      ...
      expect(result.issues).toContain("Plugin requires TUIX version 2.0.0, but current version is 1.0.0")
    })
    ...
    test("checks dependency versions", () => {
      ...
      const loadedPlugins = new Map([
        ["other-plugin", { name: "other-plugin", version: "1.0.0" }]
      ])

      const result = checkPluginCompatibility(plugin, "1.0.0", loadedPlugins)
      expect(result.compatible).toBe(false)
      expect(result.issues).toContain("Dependency other-plugin requires version 2.0.0, but 1.0.0 is loaded")
    })
