...
export class PluginRegistry {
    ...
    if (options.validateDependencies) {
      ...
      if (!validation.valid) {
        if (validation.missing.length > 0) {
          console.error(
            `Plugin '${name}' has missing dependencies: ${validation.missing.join(', ')}`
          )
        }
        if (validation.circular.length > 0) {
          console.error(
            `Plugin '${name}' has circular dependencies: ${validation.circular.join(' -> ')}`
          )
        }
  ...
  async unregister(name: string): Promise<boolean> {
    ...
    if (registered.dependents.length > 0) {
      console.warn(
        `Cannot unregister plugin '${name}' - required by: ${registered.dependents.join(', ')}`
      )
      return false
    }
