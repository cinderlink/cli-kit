          ...
          Effect.gen(function* () {
            ...
            if (scope?.handler) {
              yield* Effect.promise(() => 
                Promise.resolve(scope.handler!(msg.context))
              )
            }
            ...
            Effect.succeed({ type: "ResetCommandContext" } as AppMsg)
          ))
        ]
      ]
    ...
    case "UnregisterPlugin":
      ...
      if (pluginToRemove) {
        ...
        if (pluginToRemove.dependents.length > 0) {
          console.error(`Cannot unregister '${msg.name}': required by ${pluginToRemove.dependents.join(', ')}`)
          return [model, []]
        }
