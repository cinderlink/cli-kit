...
async function activateScopesForCommand(commandPath: string[]): Promise<void> {
  ...
  await Effect.runPromise(
    scopeManager.activateScope(cliScope.id).pipe(Effect.catchAll(() => Effect.void))
  )
  ...
  for (const segment of commandPath) {
    ...
    if (childScope) {
      // Activate this scope
      await Effect.runPromise(
        scopeManager.activateScope(childScope.id).pipe(Effect.catchAll(() => Effect.void))
      )
 ...
 */
async function renderHelpAndExit(
  AppComponent: (() => JSX.Element) | JSX.Element,
  scopePath?: string[]
): Promise<void> {
  ...
  try {
    ...
    if (scopePath && scopePath.length > 0) {
      ...
      const targetScope = allScopes.find(scope => {
        ...
        return (
          scope.path.join(' ') === scopePath.join(' ') ||
...
async function executeCommand(
  ...
  try {
    ...
    if (result && typeof result === 'object' && 'render' in result) {
      const output = await Effect.runPromise(result.render().pipe(Effect.provide(LiveServices)))
