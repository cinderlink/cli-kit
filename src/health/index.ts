...
export async function runHealthChecks(
  ...
  const overall = summary.errors > 0 ? 'errors' : 
                 summary.warnings > 0 ? 'warnings' : 'healthy'
...
export async function detectProjectType(): Promise<{
  ...
  try {
    ...
    hasTypeScript = !!(
      ...
      await fs.access(path.join(process.cwd(), 'tsconfig.json')).then(() => true).catch(() => false)
    )
    ...
    if (await fs.access(path.join(process.cwd(), 'bun.lockb')).then(() => true).catch(() => false)) {
      packageManager = 'bun'
    } else if (await fs.access(path.join(process.cwd(), 'yarn.lock')).then(() => true).catch(() => false)) {
      packageManager = 'yarn'
    } else if (await fs.access(path.join(process.cwd(), 'pnpm-lock.yaml')).then(() => true).catch(() => false)) {
      packageManager = 'pnpm'
    } else if (await fs.access(path.join(process.cwd(), 'package-lock.json')).then(() => true).catch(() => false)) {
      packageManager = 'npm'
    }
