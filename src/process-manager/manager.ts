...
interface ManagedProcess {
  ...
  startTime?: Date
  restarts: number
  status: 'running' | 'stopped' | 'errored' | 'starting' | 'stopping' | 'crashed'
  lastError?: string
  logs: ProcessLog[]
  metrics?: {
    memory?: number
...
export class ProcessManager {
  ...
  async updateProcessEnv(name: string, envUpdates: Record<string, string>, restart: boolean = false): Promise<void> {
    const managedProcess = this.processes.get(name)
    if (!managedProcess) {
      throw new Error(`Process ${name} not found`)
    }
  ...
  async startAll(): Promise<void> {
    ...
    const processes = Array.from(this.processes.values())
      .filter(p => p.config.autostart !== false)
    ...
    const results = await Promise.allSettled(
      processes.map(p => this.start(p.name))
    )
  ...
  private getProcessStatus(managedProcess: ManagedProcess) {
    return {
      ...
      uptime: managedProcess.status === 'running' && managedProcess.startTime
  ...
  private handleProcessExit(name: string, exitCode: number) {
    ...
    const shouldRestart = managedProcess.config.autorestart !== false && 
                         exitCode !== 0 && 
                         exitCode !== 143 && 
                         managedProcess.restarts < 5 && 
                         !this.isShuttingDown &&
                         !exitedQuickly // Don't restart if it exits immediately
      ...
      setTimeout(() => {
        if (!this.isShuttingDown) {
          this.start(name).catch(error => {
            this.log('error', `Failed to restart ${name}: ${error instanceof Error ? error.message : String(error)}`)
          })
        }
  ...
  private async setupLogCapture(name: string, subprocess: Subprocess, logFile: any, errorFile: any): Promise<void> {
    const managedProcess = this.processes.get(name)
    if (!managedProcess) return
    ...
    const addLogEntry = (message: string, level: 'info' | 'error' | 'debug' = 'info', source: 'stdout' | 'stderr' | 'system' = 'stdout') => {
