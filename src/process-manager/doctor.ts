...
export class ProcessDoctor {
  ...
  private async checkRunawayProcesses(report: DoctorReport) {
    ...
    for (const process of processes) {
      ...
      if (process.stats) {
        // Check memory
        if (process.config.maxMemory && process.stats.memory > process.config.maxMemory * 1024 * 1024 * 1.5) {
          const runaway: RunawayProcess = {
            name: process.name,
            pid: process.pid,
            issue: "memory",
  ...
  private analyzeErrorPatterns(errors: Array<{ message: string }>) {
    ...
    const repeating = Array.from(patterns.values())
