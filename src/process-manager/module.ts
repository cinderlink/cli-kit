...
export class ProcessManagerModule extends ModuleBase {
  ...
  startProcess(config: ProcessConfig): Effect<ProcessHandle, ProcessError> {
    return Effect.gen(function* () {
      ...
      // Simulate output
      yield* Effect.fork(
        this.simulateProcessOutput(processId)
      )
  ...
  restartProcess(processId: string): Effect<ProcessHandle, ProcessError> {
    return Effect.gen(function* () {
      ...
      yield* this.emitProcessRestarted(newHandle.id, newHandle.name, newHandle.pid, newHandle.config)
  ...
  emitProcessStopped(processId: string, processName: string, exitCode: number, config: ProcessConfig): Effect<void, never> {
    return this.emitEvent<ProcessEvent>(ProcessEventChannels.LIFECYCLE, {
  ...
  emitProcessRestarted(processId: string, processName: string, pid: number, config: ProcessConfig): Effect<void, never> {
    return this.emitEvent<ProcessEvent>(ProcessEventChannels.LIFECYCLE, {
