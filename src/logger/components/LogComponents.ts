...
export function LogMessage({
  ...
  return hstack(...parts.map((part, i) => 
    i === 0 ? part : hstack(text(" "), part)
  ))
}
  ...
  return vstack(
    ...Object.entries(metadata).map(([key, value]) => 
      hstack(
        styledText(
          " ".repeat(indent) + key + ":",
          style().foreground(Colors.blue).bold()
        ),
        text(" "),
...
export const logUtils = {
   ...
   */
  formatLogLine(level: LogLevel, message: string, options?: {
    showEmoji?: boolean
    showLevel?: boolean
    timestamp?: Date
    context?: string[]
