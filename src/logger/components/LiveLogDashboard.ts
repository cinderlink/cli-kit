...
function createFilterControls(preset: string, search: string): View {
  ...
  return vstack(
    ...
    hstack(styledText('Presets: ', style().foreground(Colors.gray)), text(presetList + '...'))
  )
}
...
function createProcessStats(
  ...
  return [styledText('Process Status:', style().foreground(Colors.cyan).bold()), ...stats, text('')]
}
...
function createFooterControls(): View {
  return vstack(
    ...
    hstack(
      ...
      styledText('1', style().foreground(Colors.cyan)),
      text(':errors '),
      styledText('2', style().foreground(Colors.cyan)),
      text(':warnings '),
      styledText('3', style().foreground(Colors.cyan)),
      text(':debug '),
      styledText('4', style().foreground(Colors.cyan)),
      text(':vite '),
      styledText('5', style().foreground(Colors.cyan)),
      text(':vitest '),
      styledText('6', style().foreground(Colors.cyan)),
      text(':production ')
    )
  )
}
...
export function ProcessLogView({
  ...
  const header = hstack(
    ...
    searchTerm
      ? hstack(text('  '), styledText(`search: "${searchTerm}"`, style().foreground(Colors.yellow)))
      : text('')
  )
