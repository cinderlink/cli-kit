...
test('service module lifecycle events', async () => {
  ...
  // Start a service
  await Effect.runPromise(serviceModule.startService('test-terminal', 'terminal'))
...
test('config module loads and validates configuration', async () => {
  ...
  await Effect.runPromise(
    eventBus.subscribe<ConfigEvent>('config-events', event => Effect.sync(() => events.push(event)))
  )
  ...
  // Load config
  const config = await Effect.runPromise(configModule.loadConfig('test.json'))
...
test('styling module responds to terminal resize', async () => {
  ...
  await Effect.runPromise(
    eventBus.subscribe('layout-events', event => Effect.sync(() => layoutEvents.push(event)))
  )
  ...
  await Effect.runPromise(serviceModule.emitTerminalResize(120, 40))
...
test('cross-module communication: config changes trigger theme update', async () => {
  ...
  // Update theme config
  await Effect.runPromise(configModule.updateConfig('app.json', 'theme', 'dark'))
