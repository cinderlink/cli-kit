...
import { ColorProfile, toAnsiSequence } from './color'
import { BorderSide, renderBox } from './borders'
...
const applyTransform = (text: string, style: Style): string => {
  const transform = style.props.transform
 ...
 */
const alignHorizontal = (line: string, width: number, align: HorizontalAlign): string => {
 ...
 */
const alignVertical = (lines: string[], height: number, align: VerticalAlign): string[] => {
  ...
  switch (align) {
    ...
    case VerticalAlign.Middle: {
      ...
      return [...createEmptyLines(top), ...lines, ...createEmptyLines(bottom)]
    }
  }
}
...
const applyPadding = (lines: string[], style: Style): string[] => {
  const top = style.props.paddingTop || 0
  const right = style.props.paddingRight || 0
  const bottom = style.props.paddingBottom || 0
  const left = style.props.paddingLeft || 0
...
const applyBorder = (lines: string[], style: Style): string[] => {
  const border = style.props.border
  const sides = style.props.borderSides ?? BorderSide.All
 ...
 */
export const buildDecorationSequence = (style: Style): string => {
  let sequence = ''

  if (style.props.bold) sequence += BOLD
  if (style.props.faint) sequence += FAINT
  if (style.props.italic) sequence += ITALIC
  if (style.props.underline) sequence += UNDERLINE
  if (style.props.blink) sequence += BLINK
  if (style.props.inverse) sequence += INVERSE
  if (style.props.strikethrough) sequence += STRIKETHROUGH
 ...
 */
const applyColors = (text: string, style: Style, colorProfile: ColorProfile): string => {
  let sequence = ''

  // Add foreground color
  const fg = style.props.foreground
  if (fg) {
    sequence += toAnsiSequence(fg, colorProfile, false)
  }

  // Add background color
  const bg = style.props.background
  if (bg) {
 ...
 * @returns ANSI escape sequence string
 */
export const toAnsiStyleCode = (style: StyleProps, colorProfile: ColorProfile): string => {
  let sequence = ''
  ...
  // Add text decorations
  sequence += buildDecorationSequence({ props: style } as Style)
...
export const renderStyled = (
  ...
  Effect.gen(function* (_) {
    ...
    const styleWidth = style.props.width
    const maxWidth = style.props.maxWidth
    ...
    const styledLines = lines.map(line => applyColors(line, style, colorProfile))

    // Apply margin
    const marginTop = style.props.marginTop
    const marginBottom = style.props.marginBottom
    const marginLeft = style.props.marginLeft
    if (marginTop || marginBottom || marginLeft) {
      ...
      const result = [...createEmptyLines(top), ...margined, ...createEmptyLines(bottom)]
