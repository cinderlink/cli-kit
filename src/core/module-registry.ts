...
export class ModuleRegistry {
  ...
  registerMany(modules: ModuleBase[]): Effect<void, ModuleError> {
    return Effect.all(
      modules.map(module => this.register(module))
    ).pipe(Effect.asVoid)
  }
    ...
    return Effect.gen(function* () {
      ...
      for (const [name, entry] of entries) {
        if (!entry.initialized) {
          yield* entry.module.initialize().pipe(
            Effect.catchAll(error => Effect.fail(new ModuleError(
              name,
      ...
      yield* Effect.all(
        entries.map(([_, entry]) => entry.module.waitForReady())
      )
    }.bind(this))
  }
  ...
  requireModule<T extends ModuleBase>(name: string): Effect<T, ModuleError> {
    return Effect.gen(function* () {
      ...
      if (!module) {
        yield* Effect.fail(new ModuleError(
          name,
          `Required module '${name}' not found`
        ))
      }
      return module
    }.bind(this))
  ...
  shutdown(): Effect<void, never> {
    return Effect.gen(function* () {
      ...
      yield* Effect.all(
        modules.map(entry => entry.module.shutdown())
      )
  ...
  waitForAllReady(): Effect<void, ModuleError> {
    return Effect.all(
      Array.from(this.modules.values()).map(entry => 
        entry.module.waitForReady()
      )
    ).pipe(Effect.asVoid)
  }
}
