...
import { KeyType } from "@core/terminal/input/keys"
    ...
    it("should handle input buffering", async () => {
      const result = await Effect.runPromise(
        Effect.gen(function* () {
          ...
          // Read from queue
          const items = yield* Stream.fromQueue(queue)
    ...
    it("should handle invalid input gracefully", async () => {
      const result = await Effect.runPromise(
        Effect.gen(function* () {
          ...
          return yield* Stream.take(invalidStream, 1).pipe(
            Stream.runCollect,
            Effect.either
          )
        }).pipe(Effect.provide(InputServiceLive))
    ...
    it("should handle high-frequency input", async () => {
      ...
      await Effect.runPromise(
        Effect.gen(function* () {
          ...
          return yield* Stream.fromIterable(events)
            .pipe(Stream.take(1000), Stream.runCollect)
        }).pipe(Effect.provide(InputServiceLive))
      )
