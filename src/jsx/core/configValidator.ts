...
/**
 * Validate JSX configuration files and detect common issues
 *
 * Analyzes tsconfig.json, bunfig.toml, package.json, and jsx-runtime files
 * to identify configuration problems that could prevent JSX from working properly.
 *
 * @param projectRoot - Root directory of the project to validate (defaults to cwd)
 * @returns Array of configuration issues found, sorted by severity
 *
 * @example
 * ```typescript
 * const issues = validateJSXConfig('/path/to/project')
 * if (issues.length > 0) {
 *   console.log('Configuration problems found:')
  ...
  if (fs.existsSync(packageJsonPath)) {
    try {
      ...
      if (!packageJson.exports?.['./jsx-runtime']) {
        issues.push({
          level: 'error',
          message: 'Missing "./jsxRuntime" export in package.json',
          file: 'package.json',
          suggestion: 'Add exports["./jsxRuntime"] pointing to jsx-runtime.ts',
        })
      }
    } catch (_e) {
...
/**
 * Format JSX configuration issues into a human-readable report
 *
 * Takes an array of configuration issues and formats them into a structured
 * report with appropriate emojis, grouping by severity level, and including
 * suggestions for fixes.
 *
 * @param issues - Array of configuration issues to format
 * @returns Formatted string report, or success message if no issues
 *
 * @example
 * ```typescript
 * const issues = validateJSXConfig()
 * const report = formatConfigIssues(issues)
 * console.log(report)
 * // Output:
 * // ❌ Errors:
 * //   • Missing "jsx" in tsconfig.json compilerOptions
 * //     File: tsconfig.json
...
export function validateJSXElement(element: any, context: string = 'unknown'): void {
  ...
  if (typeof element.render !== 'function') {
    Effect.runSync(
      logger.error('Invalid JSX element', undefined, {
        context,
        type: typeof element,
    ...
    if (Array.isArray(element)) {
      suggestions.push(
        'Element is an array. Wrap multiple elements in a container like <vstack> or <hstack>.'
      )
    }
