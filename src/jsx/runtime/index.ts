...
import { scopeManager, type ScopeContext, type ScopeDef } from '@core/model/scope'
...
class JSXPluginRegistry {
  ...
  private declarativePlugins: Map<
    string,
    { component: Function; metadata?: Record<string, unknown>; registeredAt: Date }
  > = new Map()
  ...
  getDeclarativePlugin(
    name: string
  ): { component: Function; metadata?: Record<string, unknown>; registeredAt: Date } | undefined {
...
export const jsx = (
  ...
  switch (type) {
    ...
    case 'box':
      ...
      const boxChildren = validChildren
        .map(child => {
    ...
    case 'vstack':
      ...
      const vstackChildren = validChildren
        .map(child => {
          if (isView(child)) return child
          if (typeof child === 'string') return text(child)
          // If it's a JSX element that was already processed, it should be a View
    ...
    case 'hstack':
      ...
      const hstackChildren = validChildren
        .map(child => {
          if (isView(child)) return child
          if (typeof child === 'string') return text(child)
          // If it's a JSX element that was already processed, it should be a View
...
export namespace JSX {
  ...
  export interface IntrinsicElements {
    ...
    box: { children?: unknown; style?: Style; padding?: number; margin?: number }
