 ...
 * ```
 */
export const text = (props: Record<string, unknown>) => jsxFactory('text', props)
 ...
 * ```
 */
export const vstack = (props: Record<string, unknown>) => jsxFactory('vstack', props)
 ...
 * ```
 */
export const hstack = (props: Record<string, unknown>) => jsxFactory('hstack', props)
 ...
 * ```
 */
export const Box = (props: Record<string, unknown>) => jsxFactory('Box', props)
 ...
 * ```
 */
export const panel = (props: Record<string, unknown>) => jsxFactory('panel', props)
 ...
 * ```
 */
export const button = (props: Record<string, unknown>) => jsxFactory('button', props)
 ...
 * ```
 */
export const input = (props: Record<string, unknown>) => jsxFactory('input', props)
 ...
 */
export function createJSXApp(
  AppComponent: (() => JSX.Element) | JSX.Element,
  config?: JSXAppConfig & {
    interactive?: boolean | Partial<import('../core/runtime/interactive').InteractiveConfig>
  }
): Promise<void>
export function createJSXApp<Model, Msg>(config: {
  ...
  subscriptions?: (
    model: Model
  ) => Effect<import('../core/types').Sub<Msg>, never, import('../core/types').AppServices>
}): Promise<void>
export function createJSXApp<Model = {}, Msg = never>(
  ...
  if (
    ...
    'init' in configOrComponent &&
    'update' in configOrComponent &&
    'view' in configOrComponent
  ) {
    // MVU-style app
    ...
    let component: import('../core/types').Component<Model, Msg> = {
      init: () =>
        Effect.map(Effect.succeed(mvuConfig.init()), ([model, effects]) => [
          model,
          effects.map(eff => eff as import('../core/types').Cmd<Msg>),
        ]),
      update: (msg, model) =>
        Effect.map(Effect.succeed(mvuConfig.update(msg, model)), ([newModel, effects]) => [
          newModel,
          effects.map(eff => eff as import('../core/types').Cmd<Msg>),
        ]),
      view: model => {
    ...
    // Run the MVU app
    return Effect.runPromise(runApp(component).pipe(Effect.provide(LiveServices), Effect.orDie))
  }
  ...
  const getAppElement = () => {
    try {
      if (typeof AppComponent === 'function') {
        ...
        if (!result) {
          Effect.runSync(
            logger.error('[render] ERROR: App component function returned null/undefined')
          )
  ...
  return Effect.runPromise(
    ...
    }).pipe(Effect.provide(InteractiveContextLive), Effect.provide(LiveServices), Effect.orDie)
  ).finally(cleanup)
}
...
export function render(AppComponent: (() => JSX.Element) | JSX.Element): Promise<void> {
  ...
  if (renderCount > MAX_FPS) {
    Effect.runSync(
      logger.error(`[render] ERROR: Too many render calls (${renderCount}) in short time period!`)
    )
    Effect.runSync(
      logger.error(
        '[render] This indicates a runaway render loop - terminating to prevent system hang'
      )
    )
  ...
  if (renderCount > MAX_TOTAL_RENDERS) {
    Effect.runSync(
      logger.error(`[render] ERROR: Maximum render calls (${MAX_TOTAL_RENDERS}) exceeded!`)
    )
    Effect.runSync(
      logger.error(
        '[render] This indicates a severe runaway render issue - terminating immediately'
      )
    )
    // Use Effect-based exit for critical errors
    Effect.runSync(Interactive.exit(1))
  ...
  // Initialize finalComponent outside the try block
  ...
  try {
    ...
    if (process.env.TUIX_DEBUG === 'true' && process.env.TUIX_DEBUG_AUTO_WRAP !== 'false') {
      ...
      try {
        ...
        finalComponent = () =>
          DebugWrapper({
            children: typeof AppComponent === 'function' ? AppComponent() : AppComponent,
          })
      } catch (error) {
...
// For JSX namespace, ensure it's imported or defined elsewhere
