...
export interface ScopeProps {
  ...
  }) => JSX.Element | View | Promise<JSX.Element | View>
  children?: JSX.Element | JSX.Element[]

  // Internal state
  commands?: Record<string, ScopeDef>
  args?: Record<string, ScopeDef>
  flags?: Record<string, ScopeDef>
...
export function generateScopeHelp(scope: ScopeContext): View {
  ...
  // Title
  const title =
    scope.type === 'cli' ? `${scope.name}${scope.version ? ` v${scope.version}` : ''}` : scope.name
  ...
  if (Object.keys(scope.args).length > 0) {
    ...
    Object.entries(scope.args).forEach(([name, arg]: [string, ScopeDef]) => {
      elements.push(
        jsxFactory('hstack', {
          gap: 2,
          children: [
  ...
  if (Object.keys(scope.flags).length > 0) {
    ...
    Object.entries(scope.flags).forEach(([name, flag]: [string, ScopeDef]) => {
      const flagText = flag.alias ? `-${flag.alias}, --${name}` : `--${name}`
