...
export class StylingModule extends ModuleBase {
  ...
  private handleConfigEvent(event: BaseEvent): Effect<void, never> {
    return Effect.gen(function* () {
      if (event.type === 'config-updated' && 'section' in event) {
        const section = (event as any).section as string
        if (section === 'theme') {
          const themeName = (event as any).value as string
          yield* this.setTheme(themeName)
        }
      }
    }.bind(this))
  }
  ...
  setTheme(themeName: string): Effect<void, ThemeError> {
    return Effect.gen(function* () {
      ...
      if (!theme) {
        return yield* Effect.fail(new ThemeError(
   ...
   */
  private applyResponsiveStyles(styleDef: StyleDef | undefined, size: { width: number; height: number }): Partial<ComputedStyles> {
  ...
  emitLayoutInvalidated(component: string, reason: 'resize' | 'style-change' | 'hierarchy-change'): Effect<void, never> {
