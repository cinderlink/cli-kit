...
// Types
export type Validator<T = any> = (value: T, form?: FormValues) => string | null | Promise<string | null>
...
export interface FormStore {
  ...
  dirtyFields: { value: string[] }
  touchedFields: { value: string[] }
...
export function createFormStore<T extends FormConfig>(config: T): FormStore {
  ...
  const values = $derived(() => {
    ...
    for (const [name, field] of Object.entries(fields)) {
      formValues[name] = field.value.value
    }
    return formValues
  })
  ...
  const isValidating = $derived(() => {
    return Object.values(fields).some(field => field.validating.value)
  })
  
  const dirtyFields = $derived(() => {
    return Object.entries(fields)
      .filter(([_, field]) => field.dirty.value)
      .map(([name]) => name)
  })
  
  const touchedFields = $derived(() => {
    return Object.entries(fields)
      .filter(([_, field]) => field.touched.value)
      .map(([name]) => name)
  })
  ...
  for (const [name, config] of Object.entries(fieldConfigs)) {
    if (config.dependsOn && config.compute) {
      $effect(() => {
        ...
        const shouldUpdate = config.dependsOn!.some(dep => 
          fields[dep]?.value.value !== undefined
        )
        
        if (shouldUpdate) {
          const computed = config.compute!(currentValues)
          if (fields[name].value.value !== computed) {
            fields[name].value.value = computed
          }
        }
      })
    }
  ...
  const resetField = (name: string) => {
    ...
    const initialValue = config.compute ? 
      config.compute(initialValues) : 
      config.value
...
export const formValidators = {
  ...
  min: (min: number, message?: string): Validator<number> =>
    (value) => value >= min ? null : message || `Must be at least ${min}`,
    
  max: (max: number, message?: string): Validator<number> =>
    (value) => value <= max ? null : message || `Must be at most ${max}`,
    
  matches: (fieldName: string, message = 'Fields must match'): Validator =>
    (value, form) => {
      if (!form) return null
      return value === form[fieldName] ? null : message
    },
    
  unique: (existingValues: any[], message = 'This value already exists'): Validator =>
    (value) => existingValues.includes(value) ? message : null,
    
  custom: <T>(fn: (value: T, form?: FormValues) => boolean, message: string): Validator<T> =>
    (value, form) => fn(value, form) ? null : message
}
