...
import type { Component, View, Cmd, AppServices, MouseEvent } from "../core/types"
import { MouseRouterService } from "../services/mouse-router"
import { type ComponentBounds, createBounds } from "../services/hit-test"
import { text, vstack, styledText } from "../core/view"
...
export const makeMouseAware = <Model, Msg>(
  ...
  update(msg: MouseAwareMsg<Msg>, model: MouseAwareModel<Model>) {
    // Helper to wrap inner commands
    const wrapInnerCmds = <T>(cmds: Cmd<T>[]): Cmd<MouseAwareMsg<T>>[] =>
      cmds.map(cmd => cmd.pipe(Effect.map(msg => ({ _tag: "InnerMsg" as const, msg }))))
    
    // Helper to handle mouse events
    const handleMouseEvent = (event: MouseEvent, x: number, y: number) => {
      const innerMsg = config.handleMouse(event, x, y, model.inner)
      if (!innerMsg) return Effect.succeed([model, []])
    ...
    switch (msg._tag) {
      ...
      case "MouseEnter": {
        ...
        if (!innerMsg) return Effect.succeed([newModel, []])
      ...
      case "MousePress": {
        ...
        const event: MouseEvent = {
          type: 'press', button: 'left', x: msg.x, y: msg.y,
          ctrl: false, alt: false, shift: false
        }
        
        return handleMouseEvent(event, msg.x, msg.y).pipe(
          Effect.map(([m, cmds]) => [
      ...
      case "MouseRelease": {
        const event: MouseEvent = {
          type: 'release', button: 'left', x: msg.x, y: msg.y,
          ctrl: false, alt: false, shift: false
        }
      ...
      case "MouseMove": {
        const event: MouseEvent = {
          type: 'motion', button: 'none', x: msg.x, y: msg.y,
          ctrl: false, alt: false, shift: false
        }
  ...
  view(model: MouseAwareModel<Model>): View {
    ...
    if (process.env.DEBUG_MOUSE) {
      const debugInfo = vstack(
        styledText(`Hover: ${model.mouseState.isHovered}`, style()),
        styledText(`Press: ${model.mouseState.isPressed}`, style()),
        styledText(`Pos: ${model.mouseState.lastX},${model.mouseState.lastY}`, style())
      )
