...
describe("Core Types", () => {
  ...
  describe("Component Interface", () => {
    ...
    const testComponent: Component<TestModel, TestMsg> = {
      init: Effect.succeed([{ count: 0 }, [] as const] as const),
      
      update: (msg, model) => {
        switch (msg.type) {
          case 'increment':
            return Effect.succeed([{ count: model.count + 1 }, [] as const] as const)
          case 'decrement':
            return Effect.succeed([{ count: model.count - 1 }, [] as const] as const)
        }
      },
    ...
    it("init returns model and commands", () => {
      const [model, commands] = runSimpleEffect(testComponent.init)
      expect(model.count).toBe(0)
      expect(commands).toHaveLength(0)
    })

    it("update processes messages correctly", () => {
      const initialModel = { count: 5 }
      const [updatedModel, commands] = runSimpleEffect(
  ...
  describe("Utility Types", () => {
    ...
    const testComponent: Component<TestModel, TestMsg> = {
      init: Effect.succeed([{ value: "test" }, [] as const] as const),
      update: (msg, model) => Effect.succeed([model, [] as const] as const),
  ...
  describe("Effect and Stream Types", () => {
    ...
    it("runs command effect", () => {
      const cmd: Cmd<string> = Effect.succeed("test message")
      const result = runSimpleEffect(cmd)
  ...
  describe("Complex Component Scenarios", () => {
    ...
    it("handles complex state updates", () => {
      const [initialModel] = runSimpleEffect(complexComponent.init)
      
      const [modelAfterAdd] = runSimpleEffect(
