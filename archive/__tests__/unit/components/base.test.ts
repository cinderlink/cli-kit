...
import type { KeyEvent } from "../../../../src/core/types"
import { style, Colors } from "../../../../src/styling/index"
  ...
  describe("mergeStyles", () => {
    it("merges two style objects", () => {
      const base: ComponentStyles = {
        base: style().foreground(Colors.white),
        focused: style().bold(),
        disabled: style().faint()
      }
      
      const overrides: ComponentStyles = {
        base: style(), // Required base property
        focused: style().foreground(Colors.yellow),
        hover: style().underline(),
        disabled: style().faint() // Required disabled property
      }
