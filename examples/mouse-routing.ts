...
const simpleButton = (label: string, width: number = 12): Component<SimpleButtonModel, SimpleButtonMsg> => ({
  ...
  view(model: SimpleButtonModel) {
    ...
    return box(
      styledText(finalLabel, buttonStyle),
...
const mouseRoutingDemo: Component<DemoModel, DemoMsg> = {
  ...
  view(model: DemoModel) {
    const title = styledText("🖱️  Mouse Routing Demo", style().foreground(Colors.brightYellow).bold())
    const subtitle = styledText("Click the buttons and watch mouse events being routed!", style().foreground(Colors.gray))
    ...
    const mouseInfo = box(
      vstack(
        styledText("Mouse Position:", style().foreground(Colors.brightCyan)),
        styledText(`X: ${model.mousePosition.x}, Y: ${model.mousePosition.y}`, style().foreground(Colors.white)),
        text(""),
        styledText("Mouse States:", style().foreground(Colors.brightCyan)),
        styledText(`Button 1 - Hovered: ${model.button1.mouseState.isHovered}, Pressed: ${model.button1.mouseState.isPressed}`, style().foreground(Colors.white)),
        styledText(`Button 2 - Hovered: ${model.button2.mouseState.isHovered}, Pressed: ${model.button2.mouseState.isPressed}`, style().foreground(Colors.white)),
        styledText(`Button 3 - Hovered: ${model.button3.mouseState.isHovered}, Pressed: ${model.button3.mouseState.isPressed}`, style().foreground(Colors.white))
      )
    )
    ...
    const clickInfo = box(
      vstack(
        styledText("Click Counts:", style().foreground(Colors.brightGreen)),
        styledText(`Button 1: ${model.button1.inner.clickCount}`, style().foreground(Colors.white)),
        styledText(`Button 2: ${model.button2.inner.clickCount}`, style().foreground(Colors.white)),
        styledText(`Button 3: ${model.button3.inner.clickCount}`, style().foreground(Colors.white)),
        styledText(`Total: ${model.totalClicks}`, style().foreground(Colors.brightWhite).bold())
      )
    )
    ...
    // Help
    const help = vstack(
      styledText("Controls:", style().foreground(Colors.yellow)),
      styledText("Mouse: Move around and click buttons", style().foreground(Colors.gray)),
...
const config: AppOptions = {
  fps: 30,
  debug: false,
  mouse: true, // Enable mouse support for this demo
