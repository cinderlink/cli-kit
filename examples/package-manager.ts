...
const { vstack, hstack, text, styledText } = View
import { styledBox as box } from "@/layout/box.ts"
...
const packageManager: Component<Model, Msg> = {
  ...
  view(model: Model) {
    const title = styledText("Package Manager 📦", style().foreground(Colors.brightMagenta).bold())
    const subtitle = styledText(`${mockPackages.length} packages available • ${model.selectedPackages.size} selected`, style().foreground(Colors.gray))
    ...
    switch (model.activeTab) {
      case 'packages':
        ...
        content = box(
          vstack(
            styledText("Available Packages", style().foreground(Colors.brightWhite)),
            text(""),
            packagesTableComponent.view(model.packagesTable)
          ),
          {
            border: Borders.Normal,
            borderStyle: style().foreground(Colors.brightBlue),
      ...
      case 'dependencies':
        ...
        content = box(
          vstack(
            styledText("Project Dependencies", style().foreground(Colors.brightWhite)),
            text(""),
            dependenciesTableComponent.view(model.dependenciesTable)
          ),
          {
            border: Borders.Normal,
            borderStyle: style().foreground(Colors.brightGreen),
            padding: { top: 0, right: 1, bottom: 0, left: 1 },
            width: 75,
            height: 12
          }
      ...
      case 'search':
        ...
        content = box(
          vstack(
            styledText("Search Packages", style().foreground(Colors.brightWhite)),
            text(""),
            styledText("Enter package name or keyword:", style().foreground(Colors.gray)),
            searchComponent.view(model.searchInput),
            text(""),
            styledText(model.searchTerm ? `Searching for: "${model.searchTerm}"` : "Type to search packages", style().foreground(Colors.yellow))
          ),
          {
            border: Borders.Normal,
            borderStyle: style().foreground(Colors.brightYellow),
            padding: { top: 0, right: 1, bottom: 0, left: 1 },
            width: 60,
            height: 8
          }
    ...
    const packageDetails = selectedRow ? (() => {
      ...
      return box(
        vstack(
          styledText("Package Details", style().foreground(Colors.brightCyan)),
          text(""),
          styledText(`Name: ${pkg.name}`, style().foreground(Colors.white)),
          styledText(`Version: ${pkg.version}`, style().foreground(Colors.white)),
          styledText(`Author: ${pkg.author}`, style().foreground(Colors.white)),
          styledText(`License: ${pkg.license}`, style().foreground(Colors.white)),
          styledText(`Dependencies: ${pkg.dependencies}`, style().foreground(Colors.white)),
          styledText(`Size: ${pkg.size}`, style().foreground(Colors.white)),
          text(""),
    ...
    const help = vstack(
      ...
      styledText("Tab/1-3: Switch tabs  |  ↑↓: Navigate  |  Space: Select/Toggle  |  Enter: Details", style().foreground(Colors.gray)),
      styledText("I: Install  |  U: Uninstall  |  Shift+U: Update  |  B: Bulk install  |  Ctrl+C: Exit", style().foreground(Colors.gray))
    )
    
    const mainContent = model.activeTab === 'packages' && selectedRow
      ? hstack(content, styledText("  ", style()), packageDetails)
      : content
    ...
    Effect.gen(function* (_) {
      ...
      return input.mapKeys(key => {
        ...
        if (key.key === 'tab') {
          ...
          return { tag: "switchTab", tab: tabs[nextIndex] } as const
        }
        
        // Direct tab access
        if (key.key === '1') return { tag: "switchTab", tab: 'packages' as const } as const
        if (key.key === '2') return { tag: "switchTab", tab: 'dependencies' as const } as const
        if (key.key === '3') return { tag: "switchTab", tab: 'search' as const } as const
        
        // Package operations
        if (key.key === 'i') return { tag: "installPackage" } as const
        if (key.key === 'u') return { tag: "uninstallPackage" } as const
        if (key.key === 'U') return { tag: "updatePackage" } as const
        if (key.key === 'b') return { tag: "bulkInstall" } as const
