...
const showcaseComponent: Component<Model, Msg> = {
  ...
  view(model: Model) {
    const title = text("Tabs Component Showcase", style(Colors.BrightWhite))
    const subtitle = text("Navigate between tabs to see different content", style(Colors.Gray))
    ...
    // Instructions
    const instructions = [
      text("Navigation Instructions:", style(Colors.Yellow)),
      text("← → Arrow keys  |  Tab/Shift+Tab  |  Number keys (1-5)  |  Ctrl+C to exit", style(Colors.Gray))
    ]
    ...
    const tabInfo = activeTab ? [
      text("", style()),
      text(`Active Tab: ${activeTab.icon || ""} ${activeTab.title}${activeTab.badge ? ` (${activeTab.badge})` : ""}`, style(Colors.Cyan))
    ] : []
    
    // Features demonstration
    const features = [
      text("Component Features:", style(Colors.Yellow)),
      text("✓ Multiple content types", style(Colors.Green)),
      text("✓ Icons and badges", style(Colors.Green)),
      text("✓ Keyboard navigation", style(Colors.Green)),
      text("✓ Number key shortcuts", style(Colors.Green)),
      text("✓ Focus management", style(Colors.Green)),
      text("○ Mouse support (planned)", style(Colors.Gray))
    ]
