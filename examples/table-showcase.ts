...
const showcaseComponent: Component<Model, Msg> = {
  ...
  view(model: Model) {
    const title = styledText("Table Component Showcase", style().foreground(Colors.brightWhite))
    const subtitle = styledText("Navigate with arrows, Space to select, Enter to toggle", style().foreground(Colors.gray))
    ...
    const controls = [
      ...
      styledText("↑/↓ Navigate rows  |  Space/Enter Select row  |  PgUp/PgDn Page navigation", style().foreground(Colors.gray)),
      styledText("Ctrl+A Select all  |  Esc Clear selection  |  Click column headers to sort", style().foreground(Colors.gray)),
      styledText("F Filter rows  |  C Clear filters", style().foreground(Colors.gray))
    ]
    ...
    const filterUI = model.showingFilters ? [
      ...
      box(
        vstack(
          styledText("Filter Options", style().foreground(Colors.yellow)),
          text(""),
          styledText(`Column: ${model.filterColumn}`, style().foreground(Colors.white)),
          styledText(`Value: ${model.filterValue}`, style().foreground(Colors.white)),
          text(""),
          styledText("Available columns: name, email, role, status", style().foreground(Colors.gray)),
          styledText("Press Enter to apply filter, Esc to cancel", style().foreground(Colors.gray))
        ),
        {
          width: 50,
          height: 9,
    ...
    const activeFilters = model.userTable.filters.length > 0 ? [
      text(""),
      styledText("Active Filters:", style().foreground(Colors.yellow)),
      ...model.userTable.filters.map(filter =>
        styledText(`• ${filter.column} contains "${filter.value}"`, style().foreground(Colors.cyan))
      )
    ] : []
    
    // Sample data info
    const dataInfo = [
      styledText("Sample Data:", style().foreground(Colors.yellow)),
      styledText("10 users with various roles and statuses", style().foreground(Colors.gray)),
      styledText("Try sorting by clicking column names, filtering, and selecting rows", style().foreground(Colors.gray))
    ]
