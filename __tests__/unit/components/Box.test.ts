...
describe("Box Component", () => {
  ...
  describe("Component Logic", () => {
    test("initializes with empty content", async () => {
      const [initialModel] = await Effect.runPromise(Box.init)
      expect(initialModel.content).toBe("")
      expect(initialModel.padding).toBe(1)
    })

    test("updates content through SetContent message", async () => {
      const initialModel = { content: "", padding: 1 }
      const msg = { _tag: "SetContent" as const, content: "Updated" }
      
      const [newModel] = await Effect.runPromise(Box.update(msg, initialModel))
      expect(newModel.content).toBe("Updated")
      expect(newModel.padding).toBe(1)
    })

    test("preserves padding during content updates", async () => {
  ...
  describe("Component Integration", () => {
    test("full workflow: init -> update -> view", async () => {
      ...
      const [updatedModel] = await Effect.runPromise(Box.update(msg, initialModel))
    ...
    test("multiple content updates", async () => {
      let model = { content: "", padding: 1 }
      
      const contents = ["First", "Second", "Third"]
      
      for (const content of contents) {
        const msg = { _tag: "SetContent" as const, content }
        const [newModel] = await Effect.runPromise(Box.update(msg, model))
        model = newModel
