...
import { KeyUtils, View as ViewUtils } from "@tuix/core"
import { style, Colors, Borders, renderStyledSync, type Style } from "@tuix/styling"
import {
  type UIComponent,
...
/**
 * Helper to update cursor and viewport together to reduce repetition
 */
...
/**
 * Helper to pad text to a specific width using proper Unicode-aware width calculation
 */
const padText = (text: string, width: number): string => {
  const textWidth = stringWidth(text)
  const paddingNeeded = Math.max(0, width - textWidth)
  return text + ' '.repeat(paddingNeeded)
}

/**
 * Helper to create empty line of specific width
 */
const createEmptyLine = (width: number): string => {
  return ' '.repeat(width)
}
  ...
  view(model: ListModel<T>): View {
    ...
    if (model.filteredIndices.length === 0) {
      ...
      for (let i = model.viewportStart; i < viewportEnd; i++) {
        ...
        lines.push(renderStyledSync(padText(itemText, model.width), itemStyle))
      }
      
      // Fill remaining viewport
      const remainingLines = model.viewportHeight - (viewportEnd - model.viewportStart)
      for (let i = 0; i < remainingLines; i++) {
        lines.push(createEmptyLine(model.width))
      }
    }
