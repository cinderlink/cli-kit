...
import { useState, useEffect } from "@tuix/reactive"

const { text, vstack, hstack } = ViewUtils
...
export function StreamComponent<A>(props: StreamProps<A>): JSX.Element {
        ...
        Effect.sync(() => {
          setItems(prev => {
            ...
            if (props.maxItems && newItems.length > props.maxItems) {
              return newItems.slice(-props.maxItems)
            }
            return newItems
          })
        })
  ...
  if (error && props.onError) {
    ...
    return typeof errorView === 'string' 
      ? jsx('error', { children: errorView })
      : errorView
  }
...
export function timer(
  ...
  const baseStream = Stream.iterate(0, n => n + 1).pipe(
    Stream.schedule(Schedule.spaced(interval))
  )
  
  return maxTicks 
    ? Stream.take(baseStream, maxTicks)
    : baseStream
}
...
export function random(
  ...
  return interval
    ? stream.pipe(Stream.schedule(Schedule.spaced(interval)))
    : stream
}
