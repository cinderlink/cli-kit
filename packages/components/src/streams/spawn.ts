...
// TODO: These imports need to be fixed when the process-manager and logger modules are properly defined
// import type { ProcessManager } from "@tuix/core"
// import { createConsoleLogger } from "@tuix/core"
...
export interface SpawnProps {
  ...
  stdout?: 'stream' | 'buffer' | 'ignore' | ((stream: Stream.Stream<string, never, never>) => JSX.Element)
  
  /** What to do with stderr */
  stderr?: 'stream' | 'buffer' | 'ignore' | 'merge' | ((stream: Stream.Stream<string, never, never>) => JSX.Element)
 ...
 */
function streamFromReadable(readable: ReadableStream<Uint8Array>, lineBuffer = true): Stream.Stream<string, never, never> {
...
export function SpawnComponent(props: SpawnProps): JSX.Element {
  ...
  const proc = Bun.spawn(cmd, {
    ...
    stderr: props.stderr === 'ignore' ? 'ignore' : 
            props.stderr === 'merge' ? 'inherit' : 'pipe',
    stdin: props.stdin === 'inherit' ? 'inherit' : 'pipe'
  })
  
  // Create streams from stdout and stderr
  const stdoutStream = props.stdout !== 'ignore' && proc.stdout
    ? streamFromReadable(proc.stdout, props.lineBuffer !== false)
    : Stream.empty
    
  const stderrStream = props.stderr !== 'ignore' && props.stderr !== 'merge' && proc.stderr
...
export function ManagedSpawnComponent(props: ManagedSpawnProps): JSX.Element {
    ...
    Effect.runPromise(
      Effect.promise(async () => {
        await processManager.add({
          ...
          command: Array.isArray(props.command) 
            ? props.command.join(' ') 
            : props.command,
  ...
  return SpawnComponent(spawnProps)
}
...
export function CommandPipelineComponent(props: CommandPipelineProps): JSX.Element {
  ...
  for (let i = 0; i < props.commands.length; i++) {
    ...
    if (i === 0) {
      ...
      const proc = Bun.spawn(
        Array.isArray(cmd.command) ? cmd.command : [cmd.command],
        {
  ...
  return props.children 
    ? props.children(currentStream) 
    : jsx('Stream', { stream: currentStream })
}
