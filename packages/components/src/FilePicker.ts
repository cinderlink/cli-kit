...
const createBreadcrumbs = (currentPath: string, maxWidth: number): View => {
  const normalizedPath = PathUtils.normalize(currentPath)
  const parts = normalizedPath === '/' ? [] : normalizedPath.split('/').filter(Boolean)
  let breadcrumb = '/'
  
  if (parts.length > 0) {
    const fullPath = '/' + parts.join('/')
    if (fullPath.length > maxWidth - 10) {
...
/**
 * Load directory items using the file system service
 */
const loadDirectoryItems = (path: string, fileSystemService?: FileSystemService): Effect.Effect<FileItem[], Error, never> => {
  const service = fileSystemService ?? createDefaultFileSystemService()
  return service.readDirectory(PathUtils.normalize(path))
}
  ...
  return {
    ...
    update(msg: FilePickerMsg, model: FilePickerModel) {
      switch (msg._tag) {
        ...
        case "NavigatePageUp":
          const pageSize = Math.floor((model.config.height ?? defaultConfig.height ?? 20) / 2)
          return Effect.succeed([
            {
              ...model,
        ...
        case "NavigatePageDown":
          const pageSizeDown = Math.floor((model.config.height ?? defaultConfig.height ?? 20) / 2)
          return Effect.succeed([
            {
              ...model,
        ...
        case "EnterDirectory":
          ...
          let newPath: string
          if (selectedDir.name === '..') {
            newPath = PathUtils.getParent(model.currentPath)
          } else {
            newPath = PathUtils.join(model.currentPath, selectedDir.name)
          }
    ...
    view(model: FilePickerModel): View {
      const width = model.config.width ?? defaultConfig.width ?? 80
      const height = model.config.height ?? defaultConfig.height ?? 20
