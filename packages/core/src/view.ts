 ...
 * 
 * Simple string width calculation for basic use cases that removes
 * ANSI escape sequences before measuring length.
 * 
...
// Minimal style interface for basic styling needs
// For full styling system, use @tuix/styling package
interface Style {
  get(prop: string): unknown
}

// Basic style stub that returns null for all properties
const createBasicStyle = (): Style => ({
  get: () => null
})

// Basic render function for styled text
const renderStyledSync = (content: string, style: Style): string => {
  // For core package, just return the content as-is
  // Full styling should be handled by @tuix/styling package
  return content
}
...
export const styledText = (content: string, style: Style): View => ({
  ...
  width: (style.get("width") as number) || content.split('\n').reduce((max, line) => Math.max(max, stringWidth(line)), 0),
  height: (style.get("height") as number) || content.split('\n').length
})