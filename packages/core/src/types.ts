 ...
 * ```
 */
export const TerminalService = Context.GenericTag<{
 ...
 * ```
 */
export const InputService = Context.GenericTag<{
  ...
  readonly filterKeys: (predicate: (key: KeyEvent) => boolean) => Sub<KeyEvent>
  readonly keyStream: Sub<KeyEvent>
  readonly mouseStream: Sub<MouseEvent>
}>("InputService")
 ...
 * ```
 */
export const RendererService = Context.GenericTag<{
 ...
 * ```
 */
export const StorageService = Context.GenericTag<{
...
/**
 * Mouse router service interface for routing mouse events to components
 * 
 * Provides mouse event routing capabilities to components. For full implementation,
 * use @tuix/services package.
 * 
 * @example
 * ```typescript
 * const handleMouse = Effect.gen(function* (_) {
 *   const mouseRouter = yield* _(MouseRouterService)
 *   yield* _(mouseRouter.routeMouseEvent(mouseEvent))
 * })
 * ```
 */
export const MouseRouterService = Context.GenericTag<{
  readonly routeMouseEvent: (event: MouseEvent) => Effect.Effect<unknown, never, never>
}>("MouseRouterService")

/**
 * Union of all application services.
 */
export type AppServices = TerminalService | InputService | RendererService | StorageService | MouseRouterService
